...
select pr.ttcustomer_id, pr.project_id, pr.title, pr.start_date, pr.target_date, 
 pr.resource_planning_mode, hr1.first_name as owner_first, hr1.last_name as owner_last
  from arena.t_project pr 
  join arena.t_human_resource hr1 on (pr.ttcustomer_id = hr1.ttcustomer_id and pr.owner_id = hr1.human_resource_id)
 where pr.ttcustomer_id = 979041417; 

select ta.ttcustomer_id, ta.task_id, ta.task_number, ta.project_id, ta.title, ta.orig_start_date, ta.orig_target_date, 
 ta.estimated_hours, ta.estimated_cost, ta.is_capitalized, 
 ca.title as category, ca.has_finance_entry_associations, ca.has_finances
  from arena.t_task ta 
  join arena.t_task_category ca on (ta.ttcustomer_id = ca.ttcustomer_id and ta.category_id = ca.category_id)
 where ta.ttcustomer_id = 979041417 and ta.is_active = 1; 